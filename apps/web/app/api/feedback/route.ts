...
function createEmailText(email: string, message: string, pageSource: string, timestamp: string) {
  const formattedDate = new Date(timestamp).toLocaleString('en-US', {
    dateStyle: 'full',
    timeStyle: 'long',
  });
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!pageSource || !validPageSources.includes(pageSource)) {
      return NextResponse.json(
        { success: false, error: 'Invalid page source' },
        { status: 400 }
      );
    }
    ...
    const htmlContent = createEmailHTML(email.trim(), trimmedMessage, pageSource, timestamp);
    const textContent = createEmailText(email.trim(), trimmedMessage, pageSource, timestamp);
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return NextResponse.json(
          { success: false, error: 'Failed to send feedback. Please try again later.' },
          { status: 500 }
        );
      }
      ...
      return NextResponse.json(
        { success: true, message: 'Feedback sent successfully! Thank you for your input.' },
        { status: 200 }
      );
    } catch (emailError) {
